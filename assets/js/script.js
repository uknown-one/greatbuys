...
function build(filter = "") {
  ...
  filtered.forEach((product, i) => {
    ...
    card.innerHTML = `<img src="${product.img}" alt="${product.name}"><div><h2>${product.name}</h2><p><strong>$${product.price}</strong></p><p>${product.description}</p></div><a class="btn3d" href="${product.link}" target="_blank">Buy</a>`;
